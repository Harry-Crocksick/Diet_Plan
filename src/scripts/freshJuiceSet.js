...
const freshJuiceCardContainer = document.querySelector(
  "#freshJuiceCardContainer"
);
const freshJuiceCardTemplate = document.querySelector("#freshJuiceCard");

freshJuiceSets.forEach((freshJuiceSet) =>
  freshJuiceCardContainer.appendChild(
    createFreshJuiceSet(freshJuiceSet, freshJuiceCardTemplate)
  )
);

function createFreshJuiceSet(product, template) {
...
function star(rating) {
  ...
  for (let x = 0; x < 5; x++) {
    if (x + 1 <= Math.round(rating)) {
      starsArray.push(fullStarSvgII);
    } else {
      starsArray.push(emptyStarSvgII);
    }
  }
