...
const dailySetCardContainer = document.querySelector("#dailySetCardContainer");
const dailySetCardTemplate = document.querySelector("#dailySetCard");

dailySets.forEach((dailySet) =>
  dailySetCardContainer.appendChild(
    createDailySetCard(dailySet, dailySetCardTemplate)
  )
);

function createDailySetCard(product, template) {
...
function star(rating) {
  ...
  for (let x = 0; x < 5; x++) {
    if (x + 1 <= Math.round(rating)) {
      starsArray.push(fullStarSvgIII);
    } else {
      starsArray.push(emptyStarSvgIII);
    }
  }
